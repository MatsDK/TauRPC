...
pub(super) fn export_types(
    ...
    Exporter::from(ts)
        ...
        .framework_runtime(move |mut exporter| {
            ...
            let result_map = generate_result_map(&functions);
            ...
            if !export_runtime.typed_error_impl.is_empty() {
                out.push_str(&export_runtime.typed_error_impl);
                out.push_str("\n\n");
            }
            ...
            out.push_str(match export_runtime.error_handling {
            ...
            out.push_str("})\n");
            out.push_str("export type { InferCommandOutput, TauRpcResult }\n");
...
fn generate_result_map(
    ...
    for (path, path_functions) in functions {
        ...
        for function in path_functions {
            ...
            route_map.insert(
                name,
                matches!(function.result(), Some(FunctionReturnType::Result(_, _))),
            );
        }
...
fn generate_function_field(
    ...
    let return_ty = match (error_handling, function.result()) {
        (_, Some(FunctionReturnType::Value(t))) => render_reference_dt(t, exporter)?,
        (ErrorHandlingMode::Result, Some(FunctionReturnType::Result(t, e))) => {
            let ok_ty = render_reference_dt(t, exporter)?;
            let err_ty = render_reference_dt(e, exporter)?;
            format!("TauRpcResult<{ok_ty}, {err_ty}>")
        }
        (_, Some(FunctionReturnType::Result(t, _e))) => render_reference_dt(t, exporter)?,
